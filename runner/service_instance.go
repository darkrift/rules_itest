...
func (s *ServiceInstance) Stop(sig *syscall.Signal) error {
	...
	if signal == syscall.SIGKILL {
		...
		shutdownTimeout, parseErr := time.ParseDuration(s.VersionedServiceSpec.ShutdownTimeout)
		if parseErr != nil {
			log.Printf("failed to parse health check timeout, falling back to no timeout: %v", parseErr)
			return parseErr
		}
		...
		for {
			...
			select {
			case <-waitFor:
				log.Printf("%s did not exit within %s, sending SIGKILL\n", s.Colorize(s.Label), shutdownTimeout)

				killErr := killGroup(s.cmd, syscall.SIGKILL)
				if killErr != nil {
					return err
				}
